...
function plugin.process(input, ctx)
  ...
  -- A genuine Lua error inside process(), as opposed to an invalid return
  -- value: must surface as a plugin error, not crash the process.
  if mode == "raise" then error("boom") end
  if mode == "nilindex" then return input.game.nosuchmethod() end
