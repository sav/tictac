...
std::optional<chess::Move> parseMove(const chess::Board &board, std::string_view san) {
    // parseSan throws when `san` isn't a valid/unambiguous SAN move; fall back to UCI matching.
    try {
    ...
    for (const auto &m : moves) {
        if (chess::uci::moveToUci(m) == san) return m;
    }
    return std::nullopt;
}
